...
Page({
  ...
  onLoad: function (options) {
   ...
   // var where='subjectid'=>$subjectParentId,'type'=>'xuebazi','public'=>1
     app.getPageData("BoxApi/Subject/timeline",{'subjectid':subject_id,'user_id':1,'type':'xuebazi','public':1},function(data){
        if(data.pager){
  ...
  scrolltolower:function(e){
    ...
    if(totalPages>nowPage){
      var subject_id=_this.data.subject_id;
        nowPage=nowPage+1;
        app.getPageData("BoxApi/Subject/timeline",{'subjectid':subject_id,'user_id':1,'type':'xuebazi','public':1,'p':nowPage},function(data){
          if(data.list){
